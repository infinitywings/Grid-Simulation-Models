#include <iostream>
#include <vector>
#include <cmath>
#include <fstream>
#include <helics/application_api/ValueFederate.hpp>

// GridPACK inludes
#include "mpi.h"
#include <ga.h>
#include <macdecls.h>
#include "gridpack/include/gridpack.hpp"
#include "pf_app.hpp" 

int main(int argc, char **argv) {
  // Create a FederateInfo object
  helics::FederateInfo fi;

  // Select the core type, set one core per federate
  fi.coreType = helics::CoreType::ZMQ;
  fi.coreInitString = "--federates=1";

  // Logging in debug mode
  fi.setProperty(HELICS_PROPERTY_INT_LOG_LEVEL,HELICS_LOG_LEVEL_DEBUG);

  // Set Simulator resolution
  double period = 5.0;
  fi.setProperty(HELICS_PROPERTY_TIME_PERIOD,period);

  // Set some flags
  fi.setFlagOption(HELICS_FLAG_UNINTERRUPTIBLE, false);
  fi.setFlagOption(HELICS_FLAG_TERMINATE_ON_ERROR, true);
  fi.setFlagOption(HELICS_FLAG_WAIT_FOR_CURRENT_TIME_UPDATE,true);

  // Create Value Federate
  helics::ValueFederate gpk_left("gridpack",fi);
  std::cout << "HELICS GridPAKC Federate created successfully." << std::endl;

  // Registering Publications and subscriptions
  auto Va_id = gpk_left.registerPublication("Va", "complex", "V");
  auto Vb_id = gpk_left.registerPublication("Vb", "complex", "V");
  auto Vc_id = gpk_left.registerPublication("Vc", "complex", "V");
///////////////////////////////////// glm 2 /////////////////////////////////////
  auto Va_id_2 = gpk_left.registerPublication("Va_2", "complex", "V");
  auto Vb_id_2 = gpk_left.registerPublication("Vb_2", "complex", "V");
  auto Vc_id_2 = gpk_left.registerPublication("Vc_2", "complex", "V");
///////////////////////////////////////////////////////////////////////////////////
  auto Sa_id = gpk_left.registerSubscription("gld_hlc_conn/Sa", "VA");
  auto Sb_id = gpk_left.registerSubscription("gld_hlc_conn/Sb", "VA");
  auto Sc_id = gpk_left.registerSubscription("gld_hlc_conn/Sc", "VA");
////////////////////////////////////// glm 2 ////////////////////////////////
  auto Sa_id_2 = gpk_left.registerSubscription("gld_hlc_conn_2/Sa", "VA");
  auto Sb_id_2 = gpk_left.registerSubscription("gld_hlc_conn_2/Sb", "VA");
  auto Sc_id_2 = gpk_left.registerSubscription("gld_hlc_conn_2/Sc", "VA");
/////////////////////////////////////////////////////////////////////////////

  // File to store simulatio signals
  std::ofstream outFile("gpk.csv");

  // Prepare GridPACK Environement
  gridpack::Environment env(argc,argv);
  gridpack::math::Initialize(&argc,&argv);
/////////////////////////////// glm 2 fo connectiong to different buses ////////////////////////////////////
  gridpack::parallel::Communicator world;
  auto *config = gridpack::utility::Configuration::configuration();
  if (argc>1 && argv[1]) {
    config->open(argv[1], world);
  } else {
    config->open("input.xml", world);
  }
/////////////////////////////////////////////////////////////////////////////////////////////////
  gridpack::powerflow::PFApp app_A;
  gridpack::powerflow::PFApp app_B;
  gridpack::powerflow::PFApp app_C;
  //gridpack::powerflow::PFApp app_A_2;
  //gridpack::powerflow::PFApp app_B_2;
  //gridpack::powerflow::PFApp app_C_2;  
  // Enter execution mode (this transitions the federate from initialization to execution)
  gpk_left.enterExecutingMode();
  std::cout << "GridPACK Federate has entered execution mode." << std::endl;
  // Simulation Initialization
  double total_interval = 7200.0;
  double grantedtime = 0.0;

  auto Sa = std::complex<double>(0.0,0.0);
  auto Sb = std::complex<double>(0.0,0.0);
  auto Sc = std::complex<double>(0.0,0.0);
///////////////////////////////////////////////// glm 2 //////////////////////////
  auto Sa_2 = std::complex<double>(0.0,0.0);
  auto Sb_2 = std::complex<double>(0.0,0.0);
  auto Sc_2 = std::complex<double>(0.0,0.0);
////////////////////////////////////////////////////////////////////////////  
  auto Va = std::complex<double>(1.0,0.0);
  auto Vb = std::complex<double>(-0.5,-0.866025);
  auto Vc = std::complex<double>(-0.5,0.866025);
/////////////////////////////////////////// glm 2 /////////////////////
  auto Va_2 = std::complex<double>(1.0,0.0);
  auto Vb_2 = std::complex<double>(-0.5,-0.866025);
  auto Vc_2 = std::complex<double>(-0.5,0.866025);
///////////////////////////////////////////////////////////////////////////////
///////////////////////////// source /////////////////////////////////////////////  
  //auto Vsa = std::complex<double>(1.0,0.0);
  //auto Vsb = std::complex<double>(-0.5,-0.866025);
  //auto Vsc = std::complex<double>(-0.5,0.866025);
/////////////////////////////////////////////////////////////////////
  auto r120 = std::complex<double>(-0.5,-0.866025);

  // Publish Initial center voltage
  Va_id.publish(Va*138000.00);
  Vb_id.publish(Vb*138000.00);
  Vc_id.publish(Vc*138000.00);
///////////////////////////////////////////// glm 2 //////////////////////
  Va_id_2.publish(Va_2*138000.00);
  Vb_id_2.publish(Vb_2*138000.00);
  Vc_id_2.publish(Vc_2*138000.00);
/////////////////////////////////////////////////////////////////////////
  // Setting up the GridPACK Environement

  // Entering simulation loop
  while (grantedtime < total_interval) {
    // Request time
    grantedtime = gpk_left.requestTime(grantedtime+period);

    // Get Sa from gridlab-d
    Sa = Sa_id.getValue<std::complex<double>>()/100000000.0;
    Sb = Sb_id.getValue<std::complex<double>>()/100000000.0;
    Sc = Sc_id.getValue<std::complex<double>>()/100000000.0;
///////////////////////////////////////////////////////
    // Get Sa from gridlab-d_2
    Sa_2 = Sa_id_2.getValue<std::complex<double>>()/100000000.0;
    Sb_2 = Sb_id_2.getValue<std::complex<double>>()/100000000.0;
    Sc_2 = Sc_id_2.getValue<std::complex<double>>()/100000000.0;
//////////////////////////////////////////////////////////////////////////
    //std::complex<double> S_tot_a, S_tot_b, S_tot_c;
    //S_tot_a=Sa+Sa_2;
    //S_tot_b=Sb+Sb_2;
    //S_tot_c=Sc+Sc_2;
///////////////////////////// source voltage ///////////
    //std::complex<double> Vsa, Vsb, Vsc;
    //Vsa= 1.000;
    //Vsb= Vsa*r120;
    //Vsc= Vsa*r120*r120;

    // pass S's to GridPACK and get back V's
    app_A.execute(argc, argv, Va, Va_2, Sa, Sa_2);
    app_B.execute(argc, argv, Vb, Vb_2, Sb, Sb_2);
    app_C.execute(argc, argv, Vc, Vc_2, Sc, Sc_2);
    //app_A.execute(argc, argv, Va_2, Sa_2);
    //app_B.execute(argc, argv, Vb_2, Sb_2);
    //app_C.execute(argc, argv, Vc_2, Sc_2);


    // Rotate Phase A and B Voltages
    Vb = Vb * r120;
    Vc = Vc * r120 * r120;
    // File to store simulatio signals
    //std::ofstream outFile("gpk.csv");
    Vb_2 = Vb_2 * r120;
    Vc_2 = Vc_2 * r120 * r120;


    // Log boundary signals
    outFile << "Time (s): "<< grantedtime << "\n";
    outFile << "S received from Gridlab-D, Sa: " << Sa << " Sb: " << Sb << " Sc: " << Sc <<"\n";
    outFile << "S received from Gridlab-D, Sa_2: " << Sa_2 << " Sb_2: " << Sb_2 << " Sc_2: " << Sc_2 <<"\n";
    outFile << "Updated Vv by GridPACK, Va: " << Va << " Vb: " << Vb << " Vc: " << Vc << "\n"; 
    outFile << "Updated Vv by GridPACK, Va_2: " << Va_2 << " Vb_2: " << Vb << " Vc_2: " << Vc_2 << "\n\n"; 
    
    // Publish new Center Bus Voltage
    Va_id.publish(Va*138000.0);
    Vb_id.publish(Vb*138000.0);
    Vc_id.publish(Vc*138000.0);
//////////////////////////////////////// glm 2
    Va_id_2.publish(Va_2*138000.0);
    Vb_id_2.publish(Vb_2*138000.0);
    Vc_id_2.publish(Vc_2*138000.0);
  }

  outFile << "End of Cosimulation.";
  outFile.close();

  // Terminate GridPACK Math Libraries
  gridpack::math::Finalize();

  // Finalize the federate to clean up and disconnect from the HELICS core
  gpk_left.finalize();
  std::cout << "Federate finalized." << std::endl;

  return 0;
}
